...
import { Project } from 'routes/Project/components/Project'
...
describe('(Component) Project', () => {
  ...
  beforeEach(() => {
    ...
    _wrapper = shallow(<Project {..._props} />)
  })
  ...
  it('renders with an <h2> that includes Project label.', () => {
    expect(_wrapper.find('h2').text()).to.match(/Project:/)
  })

  it('renders {props.project} at the end of the sample project <h2>.', () => {
    expect(_wrapper.find('h2').text()).to.match(/5$/)
    _wrapper.setProps({ project: 8 })
